...
- **LibreOffice 渲染缓存**：覆盖 `.pptx` 源文件后重新 PDF 可能不更新（文件大小不变）。删除旧 PDF 再生成可解决。
...
- 只保留**当天**和**前一天 12:00 之后**发布的新闻，其他一律丢弃。
  - 例：今天 5 月 9 日 → 保留 5 月 8 日 12:00 之后 + 5 月 9 日全天；5 月 8 日 11:59 及更早的全部丢弃。
...
**⚠️ LibreOffice 缓存陷阱**（3 张图变成同一张）：

LibreOffice 批量转换时会复用缓存 PDF，导致不同 PPTX 生成相同的 PNG。必须逐张转换并清理缓存：

```python
import subprocess, fitz, shutil, os, glob

for pptx in ['slide1.pptx', 'slide2.pptx', 'slide3.pptx']:
    # 1. 清理 LibreOffice 缓存（lu* 目录）
    for d in glob.glob(os.path.join(os.environ.get('TEMP', '/tmp'), 'lu*')):
        shutil.rmtree(d, ignore_errors=True)
    # 2. 逐张转换 PPTX → PDF → PNG
    base = os.path.splitext(pptx)[0]
    subprocess.run(['soffice', '--headless', '--convert-to', 'pdf', '--outdir', 'data', f'data/{pptx}'])
    doc = fitz.open(f'data/{base}.pdf')
    doc[0].get_pixmap(matrix=fitz.Matrix(2, 2)).save(f'data/{base}.png')
    # 3. 验证文件大小不同（相同大小 = 缓存命中 = 重来）
```

硬性检查：上传前比对所有 PNG 文件大小，相同则回退重转。
